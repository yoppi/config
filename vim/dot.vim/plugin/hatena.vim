...
" Modified by:  yoppi http://d.hatena.ne.jp/yoppiblog/
" Version:      20070830
...
" Interface "{{{1
function! hatena#edit(...) "{{{2
  if !exists('b:hatena_login_info')
    let hatena_login_info = s:login()
  ...
  let b:titlestring_save = &titlestring
...
"TODO: この関数をグローバルにするセキュリティリスクについて考察
  ...
  let content = system(s:curl_cmd . ' "' . s:hatena_login_url . '" "-d name=' 
...
function! s:get_user() "{{{2
  if !strlen(g:hatena_user)
    let hatena_user = input('はてなユーザID(user/group:user): ', 
                            \ '', 'customlist,hatena#enum_users')
  else
...
function! s:valid_date_p(date) "{{{2
  ...
  let matches = matchlist(a:date, pat)
  if !len(matches)
...
function! s:parse_content(content) "{{{2
  ...
  let body        = s:html_unescape(matchstr(a:content, '<textarea.\{-}name="body"[^>]*>\zs.\{-}\ze</textarea>'))
  let result={}
...
function! s:html_unescape(string) "{{{2
" HTMLエスケープを解除
  let string = a:string
...
function! s:update(...) "{{{2
  ...
  if !exists('b:hatena_login_info') || !exists('b:year') || !exists('b:month') 
     \ || !exists('b:day') || !exists('b:day_title') || !exists('b:rkm')
  ...
  " ログイン
  if !exists('b:hatena_login_info')
    let b:hatena_login_info = s:login()
...
function! s:post(base_url, user, cookie_file, diary, body_file) "{{{2
  if a:body_file == ""
...
function! s:get_rkm(login_info) "{{{2
...
function! s:post_entry(login_info, rkm, entry) "{{{2
...
function! s:get_entry(login_info, rkm, entry) "{{{2
" はてなダイアリー/グループに新しいエントリを投稿する
" login_info: HatenaLogin() で得られるもの
" rkm: HatenaGetRKM() などで得られるもの
" entry: エントリの情報、以下のキーを持つ
...
function! s:parse_entries(body) "{{{2
...
" TODO: タイトルのない冒頭部
  ...
  call cursor(1,1)
  let l:es = []
  let l:e = s:find_next_entry()
  while l:e
    let l:next_e = s:find_next_entry()
    call add(l:es,[l:e,next_e?next_e-1:line('$')])
    let l:e = l:next_e
  endwhile
  let l:result = []
  for [l:st,l:ed] in l:es
    call add(result, s:parse_entry(getline(l:st),getline(l:st+1,l:ed)))
  endfor
  let &modified = 0
  close
  return l:result
